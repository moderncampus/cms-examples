<cfcomponent displayname="XML-RPC" output="false" hint="Allows painless translation of XML-RPC packets to and from CFML data structures">
	...
	ARPAN: Made some changes in here to force a few types
	--->
	...
	<cffunction name="serialize" access="public" output="false">
		...
		<cfelseif IsNumeric(dummy) AND FIND(".",dummy) eq 0 OR IsSimpleValue(dummy) AND (Left(dummy, 4) IS "$int" OR Left(dummy, 5) IS "(int)")>
		...
		<cfelseif IsDate(dummy) AND FIND(",",dummy) eq 0 OR (IsSimpleValue(dummy) AND (Left(dummy, 17) IS "$dateTime.iso8601" OR Left(dummy, 18) IS "(dateTime.iso8601)"))>
		...
		<cfelseif IsSimpleValue(dummy) OR (IsSimpleValue(dummy) AND (Left(dummy, 7) IS "$string" OR Left(dummy, 8) IS "(string)"))>
			...
			<cfset dummy = Replace(dummy,chr(237),"","ALL")>
