...
Blog Posting
Structured data for a Blog Posting should contain the following:
...
<xsl:stylesheet version="3.0" 
	...
	<!-- Blog Publisher Settings. Includes the College Name, Logo Reference, Logo Height and Logo Width -->
	<xsl:variable name="blog-publisher-settings">
		<settings>
	...
	<!-- Blog Default Image Settings, when no image is provided. Includes Type, Full Image Path, Image Height, Image Width  -->
	<xsl:variable name="blog-default-image-items">
		<defaults>
	...
	<!-- Blog Custom Image Settings. Define where the blog image should be pulled from. Includes Condition(when should this be used), Type, Full Image Path, Image Height, Image Width -->
	<xsl:variable name="blog-custom-image-items">
		<xsl:variable name="first-image" select="/document/ouc:div[@label='maincontent']/descendant::img[1]" />
		<custom>
			<type>ImageObject</type>
			<!-- condition when should we use the custom blog image, needs to be 'true' to be used -->
	...
	<!-- Check to see if Blog Author is a page parameter, otherwise pull from the author from the current user -->
	<xsl:variable name="blog-author" select="if (/document/descendant::parameter[@name='author'] != '') then 
											 /document/descendant::parameter[@name='author'] else
											 concat($ou:firstname, ' ', $ou:lastname)" />
	
	<!-- Variable to hold the blog headline or title -->
	<xsl:variable name="blog-headline" select="/document/descendant::parameter[@name='heading']" />
	
	<!-- Output the Blog Posting Structured Data -->
	<xsl:template name="blog-posting-structured-data">
		<xsl:text>&#xa;</xsl:text>
		<script type="application/ld+json">
			<!-- create a variable of XML structure to convert to JSON -->
			<xsl:variable name="blog-xml-for-json">
				<map xmlns="http://www.w3.org/2005/xpath-functions">
					<xsl:copy-of select="ou:create-string-key('@context', 'https://schema.org')" />
					<xsl:copy-of select="ou:create-string-key('@type', 'blogPosting')" />
					...
					<xsl:copy-of select="ou:create-string-key('headline', $blog-headline)" />
					<map key="image">
						<xsl:copy-of select="ou:create-string-key('@type', $blog-custom-image-items/custom/type, $blog-default-image-items/defaults/type, $blog-custom-image-items/custom/condition)" />
						<xsl:copy-of select="ou:create-string-key('url', $blog-custom-image-items/custom/image, $blog-default-image-items/defaults/image, $blog-custom-image-items/custom/condition)" />
						<xsl:copy-of select="ou:create-string-key('height', $blog-custom-image-items/custom/height, $blog-default-image-items/defaults/height, $blog-custom-image-items/custom/condition)" />
						<xsl:copy-of select="ou:create-string-key('width', $blog-custom-image-items/custom/width, $blog-default-image-items/defaults/width, $blog-custom-image-items/custom/condition)" />
					</map>
					...
					<map key="author">
						...
						<xsl:copy-of select="ou:create-string-key('name', $blog-author)" />
					</map>
					<map key="publisher">
						<xsl:copy-of select="ou:create-string-key('@type', 'Organization')" />
						<xsl:copy-of select="ou:create-string-key('name', $blog-publisher-settings/settings/name)" />
						<map key="logo">
							<xsl:copy-of select="ou:create-string-key('@type', 'ImageObject')" />
							<xsl:copy-of select="ou:create-string-key('url', $blog-publisher-settings/settings/logo)" />
							<xsl:copy-of select="ou:create-string-key('height', $blog-publisher-settings/settings/logo-height)" />
							<xsl:copy-of select="ou:create-string-key('width', $blog-publisher-settings/settings/logo-width)" />
						</map>
					</map>
				</map>
			</xsl:variable>
			<!-- convert XML structure to JSON -->
			<xsl:value-of select="xml-to-json($blog-xml-for-json)" />
		</script>
		<xsl:text>&#xa;</xsl:text>
	</xsl:template>
</xsl:stylesheet>