...
<xsl:stylesheet version="3.0"
	...
	<xsl:template name="page-content">
		<xsl:variable name="author" select="if (normalize-space(post-info/ouc:div[@label='post-author']) != '') then post-info/ouc:div[@label='post-author'] else ''"/>
		<xsl:variable name="email" select="if (normalize-space(post-info/ouc:div[@label='post-email']) != '') then post-info/ouc:div[@label='post-email'] else ''"/>
		<div class="row">
		...
		<div class="row">
			<div class="{if (ou:pcf-param('right-enable') = 'true') then 'large-8' else 'large-12'} columns">
				<p><xsl:value-of select="ou:display-long-date(ouc:properties[@label='config']/parameter[@name='date-time'])" />&nbsp;<a href="mailto:{$email}"><xsl:value-of select="$author"/></a></p>
				<!-- Main Content Region -->
	...
	<xsl:template name="template-headcode">
		...
		<xsl:if test="string-length(post-info/ouc:div[@label='author']) > 0"><meta name="Author" content="{post-info/ouc:div[@label='post-author']}"/></xsl:if>
